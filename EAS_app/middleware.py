    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            if user.user_type == '1': # Is it the HOD/Admin
                if modulename == 'EAS_app.employee_views':
                    return redirect(reverse('admin_home'))
            elif user.user_type == '2': #  Manager :-/ ?
                if modulename == 'EAS_app.employee_views' or modulename == 'EAS_app.hod_views':
                    return redirect(reverse('manager_home'))
            elif user.user_type == '3': # ... or Employee ?
                if modulename == 'EAS_app.hod_views' or modulename == 'EAS_app.manager_views':
