...
def manager_home(request):
    ...
    total_employees = Employee.objects.filter(department=manager.department).count()
    total_leave = LeaveReportManager.objects.filter(manager=manager).count()
    sections = Section.objects.filter(manager=manager)
    ...
    context = {
        'page_title': 'Manager Panel - ' + str(manager.admin.last_name) + ' (' + str(manager.department) + ')',
...
def manager_take_attendance(request):
    manager = get_object_or_404(Manager, admin=request.user)
    sections = Section.objects.filter(manager_id=manager)
...
def get_employees(request):
    ...
    try:
        ...
        employees = Employee.objects.filter(
            department_id=section.department.id, contract=contract)
        employee_data = []
...
def save_attendance(request):
    ...
    try:
        contract = get_object_or_404(Contract, id=contract_id)
        section = get_object_or_404(Section, id=section_id)
        attendance = Attendance(contract=contract, section=section, date=date)
        attendance.save()
...
def manager_fcmtoken(request):
    ...
    try:
        manager_user = get_object_or_404(AllUser, id=request.user.id)
