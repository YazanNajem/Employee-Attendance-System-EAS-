...
def employee_home(request):
    ...
    total_section = Section.objects.filter(department=employee.department).count()
    ...
    data_present = []
    data_absent = []
    sections = Section.objects.filter(department=employee.department)
...
def employee_fcmtoken(request):
    ...
    employee_user = get_object_or_404(AllUser, id=request.user.id)
    try:
